...
DEFAULT_AGENT_NAME = os.getenv(
    "DEFAULT_AGENT_NAME", meta.get("name", "Systems Manager")
)
...
def agent_template(mcp_url: str = None, mcp_config: str = None, **kwargs):
    ...
    if not effective_mcp_url and not effective_mcp_config:
        try:
            ...
            logger.info("Systems Manager: Using in-process MCP instance.")
        except (ImportError, Exception) as e:
            logger.warning(f"Systems Manager: Could not load in-process MCP: {e}")

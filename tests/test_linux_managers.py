...
def deployment_artifact_policy(monkeypatch, tmp_path):
    """Select a confined root and explicit repository/artifact trust policy."""
    ...
    for name in (
        ...
        artifact = root / name
        artifact.write_bytes(("test artifact " + name).encode())
    ...
    monkeypatch.setenv(
        ...
        json.dumps(["https://repo.example.com"]),
    )
    monkeypatch.setenv(
        "SYSTEMS_MANAGER_LOCAL_PACKAGE_SHA256_MAP", json.dumps(artifacts)
    )
    return root
...
class TestAptManager:
    ...
    @pytest.fixture
    def apt_manager(self, mock_linux_platform):
        """Create an AptManager instance for testing."""
        _ = mock_linux_platform
    ...
    def test_add_repository_success(self, apt_manager):
        ...
        result = apt_manager.add_repository("https://repo.example.com", "test-repo")
        assert result["success"] is True
    ...
    def test_add_repository_add_fails(self, apt_manager):
        ...
        result = apt_manager.add_repository("https://repo.example.com")
        assert result["success"] is False
    ...
    def test_install_local_package_success(self, apt_manager):
        ...
        result = apt_manager.install_local_package("package.deb")
        assert result["success"] is True

    def test_install_local_package_not_deb(self, apt_manager):
        """Test local package installation with non-deb file."""
        with pytest.raises(ValueError, match="Invalid local package artifact"):
            apt_manager.install_local_package("package.rpm")
    ...
    def test_get_package_info_success(self, apt_manager):
        ...
        assert result["success"] is True
        assert result["info"]["package"] == "vim"
...
class TestDnfManager:
    ...
    @pytest.fixture
    def dnf_manager(self, mock_rhel_platform):
        """Create a DnfManager instance for testing."""
        _ = mock_rhel_platform
    ...
    def test_install_applications_with_snap_fallback(self, dnf_manager):
        ...
        dnf_manager.install_via_snap = Mock(return_value={"success": True})
...
class TestZypperManager:
    ...
    def test_install_local_package_success(self, zypper_manager):
        ...
        result = zypper_manager.install_local_package("package.rpm")
        assert result["success"] is True

    def test_install_local_package_not_rpm(self, zypper_manager):
        """Test local package installation with non-rpm file."""
        with pytest.raises(ValueError, match="Invalid local package artifact"):
            zypper_manager.install_local_package("package.deb")
...
class TestPacmanManager:
    ...
    @pytest.fixture
    def pacman_manager(self, mock_arch_platform):
        """Create a PacmanManager instance for testing."""
        _ = mock_arch_platform
    ...
    def test_optimize_success(self, pacman_manager):
        ...
        assert result["success"] is True
        assert result["message"] == "No removable orphan packages found"
    ...
    def test_add_repository_success(self, pacman_manager):
        ...
        assert result["success"] is False
        assert "external governance" in result["error"]
    ...
    def test_install_local_package_success(self, pacman_manager):
        ...
        result = pacman_manager.install_local_package("package.pkg.tar.zst")
        assert result["success"] is True

...
class TestWindowsManager:
    ...
    @pytest.fixture
    def windows_manager(self, mock_windows_platform, tmp_path):
        """Create a WindowsManager instance for testing."""
        _ = mock_windows_platform
    ...
    @pytest.fixture
    def windows_manager_no_winget(self, mock_windows_platform, tmp_path):
    ...
    def test_update_success(self, windows_manager):
        ...
        assert result["success"] is True
        assert result["message"] == "Managed applications updated"
        assert result["os_update"] == "requires external Windows Update governance"
    ...
    def test_search_package_success(self, windows_manager):
        ...
        windows_manager.run_command = Mock(
            return_value={
                ...
                "stdout": (
                    "Name  Id  Version\n"
                    "Visual Studio Code  Microsoft.VisualStudioCode  1.0.0"
                ),
            }
        )
        ...
        assert result["success"] is True
        assert result["packages"][0]["name"] == "Visual Studio Code"
    ...
    def test_get_package_info_success(self, windows_manager):
        ...
        assert result["success"] is True
        assert result["info"]["name"] == "Visual Studio Code"
        ...
        windows_manager.run_command = Mock(
            return_value={
                ...
                "stdout": (
                    ...
                    "Visual Studio Code  Microsoft.VisualStudioCode  1.0.0  1.1.0\n"
    ...
    def test_clean_package_cache(self, windows_manager):
        ...
        assert result["success"] is True
        assert result["message"] == "Windows package cache cleanup completed"
    ...
    def test_winget_bin_path_expansion(self, mock_windows_platform, tmp_path):
        """Test that winget is resolved only by the trusted executable policy."""
        _ = mock_windows_platform
        with patch("os.path.exists", return_value=True):
            manager = WindowsManager(silent=True)
            assert manager.winget_bin == "winget.exe"
    ...
    def test_update_command_structure(self, windows_manager):
        ...
        assert windows_manager.run_command.call_count == 1
        ...
        assert all(
            "powershell.exe" not in call.args[0]
            for call in windows_manager.run_command.call_args_list
        )
